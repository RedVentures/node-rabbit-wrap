...
describe('queue wrapper', function() {
  ...
  describe('#ignore', function() {
    ...
    beforeEach(function(done) {
      qu = conn.queue('my.unit.test.queue.2', {
          autoDelete: true,
          durable: false
        })
        .listen({
    ...
    it('should ignore multiple consumers', function(done) {
      ...
      qu.listen({
        ...
        try {
          ...
          qu.ignore(function(err) {
            if (err) {
              return done(err);
            }
            try {
              qu.fixture.calls.length.should.eql(1);
              done();
            } catch (e) {
              done(e);
            }
  ...
  describe('#ack', function() {
    ...
    before(function(done) {
      var ex = conn.exchange('another.test.exchange', {
        type: 'direct',
        autoDelete: true,
        durable: false
      });
      qu = conn.queue('my.unit.test.queue.#ack', {
          autoDelete: true,
          durable: false
        })
...
function getNewQueue(conn, cb) {
  return conn.queue('my.unit.test.queue', {
    autoDelete: true,
    durable: false
  }, cb || function() {});
}