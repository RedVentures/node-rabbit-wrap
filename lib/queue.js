...
function Queue(conn, name, options, cb) {
    ...
    /**
     * 
     * Map function string -> consumerTag index
     * 
     */
    this.listeners = {};
...
Queue.prototype.unbindQueue = function (exchange, binding, cb) {
    var self = this;
    this.call('unbindQueue', 
        [this.name, exchange, binding], 
        false,
        cb
    );
...
Queue.prototype.listen = function (options, listener, cb) {
    ...
    function addConsumer(err, data) {
        ...
        var idx = self.consumerTags.push(data.consumerTag);
        self.listeners[fnstr] = idx;
...
Queue.prototype.ignore = function (listener, cb) {
    ...
    if (listener) {
        switch (typeof listener) {
            case 'string':
                ...
                break;
            case 'function':
                pos = this.listeners[listener.toString()];
                cancels = [this.consumerTags[pos]];
                break;
        }
    }
    ...
    function next(i, err, others) {
        ...
        self.call('cancel', [tag], false, util.curry(next, i + 1));
